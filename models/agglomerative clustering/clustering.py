...
# Step 1: Agglomerative Clustering on the first half (X_train)
agglom_train = AgglomerativeClustering(n_clusters=n_clusters, linkage='ward')
train_labels = agglom_train.fit_predict(X_train)
silhouette_train = silhouette_score(X_train, train_labels)
print(f"Silhouette Score for First 50% of Data (Agglomerative Clustering): {silhouette_train}")

# Step 2: Agglomerative Clustering on the full dataset
agglom_full = AgglomerativeClustering(n_clusters=n_clusters, linkage='ward')
full_labels = agglom_full.fit_predict(features_pca)
silhouette_full = silhouette_score(features_pca, full_labels)
print(f"Silhouette Score for Full Dataset (Agglomerative Clustering): {silhouette_full}")
...
# Plot for 50% Dataset Clustering with Agglomerative Clustering
plt.subplot(1, 3, 2)
for i in range(n_clusters):
    cluster_points = X_train[train_labels == i]
    plt.scatter(cluster_points[:, 0], cluster_points[:, 1], s=20, label=f"Cluster {i}")
plt.title(f'50% Dataset Clustering (Agglomerative, Silhouette: {silhouette_train:.2f})')
plt.xlabel("PCA Component 1")
plt.ylabel("PCA Component 2")
plt.grid()

# Plot for Full Dataset Clustering with Agglomerative Clustering
plt.subplot(1, 3, 3)
for i in range(n_clusters):
    cluster_points = features_pca[full_labels == i]
    plt.scatter(cluster_points[:, 0], cluster_points[:, 1], s=20, label=f"Cluster {i}")
plt.title(f'Full Dataset Clustering (Agglomerative, Silhouette: {silhouette_full:.2f})')
plt.xlabel("PCA Component 1")
plt.ylabel("PCA Component 2")
plt.grid()
