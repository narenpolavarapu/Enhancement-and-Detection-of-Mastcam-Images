...
# Step 1: Clustering on the first half (X_train)
...
print(f"Silhouette Score for First 50% of Data: {silhouette_train}")

# Step 2: Clustering on the full dataset
...
print(f"Silhouette Score for Full Dataset: {silhouette_full}")
...
# Plot for 50% Dataset Clustering
plt.subplot(1, 3, 2)
for i in range(n_clusters):
    cluster_points = X_train[train_labels == i]
    plt.scatter(cluster_points[:, 0], cluster_points[:, 1], s=20, label=f"Cluster {i}")
plt.title(f'50% Dataset Clustering (Silhouette: {silhouette_train:.2f})')
plt.xlabel("PCA Component 1")
plt.ylabel("PCA Component 2")
plt.grid()

# Plot for Full Dataset Clustering
plt.subplot(1, 3, 3)
for i in range(n_clusters):
    cluster_points = features_pca[full_labels == i]
    plt.scatter(cluster_points[:, 0], cluster_points[:, 1], s=20, label=f"Cluster {i}")
plt.title(f'Full Dataset Clustering (Silhouette: {silhouette_full:.2f})')
plt.xlabel("PCA Component 1")
plt.ylabel("PCA Component 2")
plt.grid()
