...
# Step 1: Clustering on the first half (X_train) with KMeans++
kmeans_train = KMeans(n_clusters=n_clusters, init="k-means++", random_state=42)
kmeans_train.fit(X_train)
train_labels = kmeans_train.labels_
silhouette_train = silhouette_score(X_train, train_labels)
print(f"Silhouette Score for First 50% of Data (KMeans++): {silhouette_train}")

# Step 2: Clustering on the full dataset with KMeans++
kmeans_full = KMeans(n_clusters=n_clusters, init="k-means++", random_state=42)
kmeans_full.fit(features_pca)
full_labels = kmeans_full.labels_
silhouette_full = silhouette_score(features_pca, full_labels)
print(f"Silhouette Score for Full Dataset (KMeans++): {silhouette_full}")
...
# Plot for 50% Dataset Clustering with KMeans++
plt.subplot(1, 3, 2)
for i in range(n_clusters):
    cluster_points = X_train[train_labels == i]
    plt.scatter(cluster_points[:, 0], cluster_points[:, 1], s=20, label=f"Cluster {i}")
plt.title(f'50% Dataset Clustering (KMeans++, Silhouette: {silhouette_train:.2f})')
plt.xlabel("PCA Component 1")
plt.ylabel("PCA Component 2")
plt.grid()

# Plot for Full Dataset Clustering with KMeans++
plt.subplot(1, 3, 3)
for i in range(n_clusters):
    cluster_points = features_pca[full_labels == i]
    plt.scatter(cluster_points[:, 0], cluster_points[:, 1], s=20, label=f"Cluster {i}")
plt.title(f'Full Dataset Clustering (KMeans++, Silhouette: {silhouette_full:.2f})')
plt.xlabel("PCA Component 1")
plt.ylabel("PCA Component 2")
plt.grid()
